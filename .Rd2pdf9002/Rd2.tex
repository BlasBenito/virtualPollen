...
\item[Imports]\AsIs{ggplot2, cowplot, viridis, mgcv}
\end{description}
\Rdcontents{\R{} topics documented:}
\inputencoding{utf8}
\HeaderA{accumulationRate}{Accumulation rate}{accumulationRate}
\keyword{datasets}{accumulationRate}
%
\begin{Description}\relax
Dataframe, output of \code{\LinkA{simulateAccumulationRate}{simulateAccumulationRate}}
\end{Description}
%
\begin{Usage}
\begin{verbatim}
...
%
\begin{Format}
dataframe with 10000 rows and the following columns:
...
%
\begin{Description}\relax
It converts all columns (but the \code{label} one) of a parameters dataframe created by \code{\LinkA{parametersDataframe}{parametersDataframe}} into type numeric, and checks the coherence of the parameters for each taxon. It provides feedback on the check results on screen for each taxon.
\end{Description}
%
\begin{Usage}
\begin{verbatim}
...
%
\begin{Value}
A dataframe like \code{\LinkA{accumulationRate}{accumulationRate}}, with the following columns:
\begin{itemize}
...
%
\begin{SeeAlso}\relax
\code{\LinkA{accumulationRate}{accumulationRate}}, \code{\LinkA{aggregateSimulation}{aggregateSimulation}}
\end{SeeAlso}
%
\begin{Examples}
\begin{ExampleCode}
...
%
\begin{Value}
A vector of the same length as \code{time}. Datasets \code{\LinkA{driverA}{driverA}} and \code{\LinkA{driverB}{driverB}} are outputs of this function.
\end{Value}
%
\begin{Author}\relax
Blas M. Benito  <blasbenito@gmail.com>
...
%
\begin{SeeAlso}\relax
\code{\LinkA{rescaleVector}{rescaleVector}}, \code{\LinkA{driverA}{driverA}}, \code{\LinkA{driverB}{driverB}}
\end{SeeAlso}
%
\begin{Examples}
\begin{ExampleCode}
...
%
\begin{Description}\relax
Wrapper of \code{simulateDriver} to generate several drivers with different autocorrelation lengths, and return a long format table to be used as input for \code{\LinkA{simulatePopulation}{simulatePopulation}}. Note that the variable \code{time} runs from left to right, with lower values representing older samples.
\end{Description}
%
\begin{Usage}
\begin{verbatim}
...
%
\begin{SeeAlso}\relax
\code{\LinkA{drivers}{drivers}}, \code{\LinkA{simulateDriver}{simulateDriver}}, \code{\LinkA{simulatePopulation}{simulatePopulation}}, \code{\LinkA{drivers}{drivers}}
\end{SeeAlso}
%
\begin{Examples}
\begin{ExampleCode}
...
\item[\code{driver.A}] numeric vector with driver values. Typically produced by \code{\LinkA{simulateDriver}{simulateDriver}}.

\item[\code{driver.B}] numeric vector with driver values.  Typically produced by \code{\LinkA{simulateDriver}{simulateDriver}}. Must have same length as \code{driver.A}.

\item[\code{drivers}] dataframe with drivers produced by \code{\LinkA{simulateDriverS}{simulateDriverS}}. It should have the columns:
\begin{itemize}
...
%
\begin{Value}
A list of dataframes, each one of them with the results of one simulation. The dataset \code{\LinkA{simulation}{simulation}} exemplifies the output of this function. Each dataframe in the output list has the columns:
\begin{itemize}
